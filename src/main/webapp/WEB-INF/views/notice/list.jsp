	        ...
	        if (listCnt > 0) {
	            ...
	            $.each(items, function (key) {
	            	...
	            	//groupAvailable == "Y" ? groupAvailable = "활성" : groupAvailable = "비활성";
	     			...
	     			sb.AppendFormat('<tr style="cursor:pointer;" onclick="javascript:fnNoticeView(\'{0}\');return false;">', no);
	    			sb.AppendFormat('  <td class="text-center">{0}</td> ', rownum);
	    			sb.AppendFormat('  <td class="text-center">{0}</td> ', title);
	    			sb.AppendFormat('  <td class="text-center">{0}</td> ', writer);
	    			sb.AppendFormat('  <td class="text-center">{0}</td> ', creDt);
	    			sb.AppendFormat('  <td class="text-center">{0}</td> ', hit);
	    			sb.Append('</tr>');
	            });
...
<!-- 
		<div class="container">
			...
			<div class="container-fluid">
			    ...
			    <form class="navbar-form navbar-left" name="frmMain" id="frmMain">
				  <div class="form-group">
				    <label for="groupName">그룹이름 &nbsp;</label>
				    <input type="text" class="form-control" id="groupName" id="groupName" >&nbsp;&nbsp;&nbsp;
				  </div>
				  <div class="form-group">
				  ...
				  <div class="form-group">
				    <label for="groupAvailable">그룹활성화 &nbsp;</label>
				      <input type="checkbox" name="groupAvailable" id="groupAvailable" >
			          <select class="form-control" name="groupAvailable" id="groupAvailable">
				...
				<div class="form-group navbar-form navbar-right">
		        	<button type="button" class="btn btn-default" id="btnSearch">조회</button>
		        </div>
				</div>
			</div>
			</nav>
